...
func main() {
	...
	// Auto-initialize if system not yet setup
	if !isInitialized(db) {
		if err := autoInitializeSystem(app); err != nil {
			fmt.Fprintln(os.Stderr, "auto-init warning:", err)
			// Continue anyway - user can setup manually via API
		} else {
	...
	mux.HandleFunc("/api/v1/setup/status", app.setupStatusHandler)
	mux.HandleFunc("/api/v1/setup/config", app.setupConfigHandler)
